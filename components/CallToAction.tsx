...
export default function CallToAction() {
  ...
  return (
    <section className="py-24 px-4 sm:px-6 lg:px-8 bg-gray-950 relative overflow-hidden" data-scroll-section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto mb-16">
          {contactMethods.map((method, index) => {
            ...
            return (
              <motion.div
                key={method.title}
                className="group relative bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-8 text-center hover:bg-white/10 transition-all duration-500 overflow-hidden"
                initial={{ opacity: 0, y: 30 }}
                animate={isInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 30 }}
              ...
              >
                ...
                <div className="relative z-10">
                  <div className={`p-4 rounded-2xl bg-gradient-to-br ${method.gradient} inline-flex mb-6`}>
                  ...
                  <Link
                    href={method.href}
                    className="group/btn inline-flex items-center justify-center px-6 py-3 bg-white/10 border border-white/20 rounded-full text-white font-medium hover:bg-white/20 transition-all duration-300"
                  >
                    {method.action}
        ...
        >
          ...
          <div className="text-center">
            <p className="text-gray-400 text-lg">
              目前状态：
              <span className="font-bold text-green-400">{ctaData.status}</span>
            </p>
